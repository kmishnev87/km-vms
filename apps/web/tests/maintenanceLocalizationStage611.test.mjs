...
for (const [lang, copy] of Object.entries(dictionaries)) {
  ...
  assert.deepEqual(
    updateApplyErrorMessages({
      category: "schema_update_failed",
      message: "Database schema preparation failed during update apply.",
      operator_action: "Review the database schema preparation failure before retrying the update.",
    }, copy, lang),
    [
      copy.maintenanceMessageLabels.schema_update_failed,
