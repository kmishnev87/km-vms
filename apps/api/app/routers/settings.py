...
from app.services.update_maintenance import UpdateMaintenanceBlocked, apply_update_maintenance, dry_run_update_maintenance
...
@router.post("/system/update/dry-run")
def system_update_dry_run(
    payload: UpdateDryRunRequest,
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission("manage_settings")),
):
    result = dry_run_update_maintenance(db, actor=current_user)
    create_event(
        db=db,
        actor=current_user,
        category="system",
        event_type="system.update_apply_dry_run_completed",
        severity="info" if result.get("status") != "blocked" else "warning",
        message_ru="Product update apply dry-run completed.",
        message_en="Product update apply dry-run completed.",
...
def system_update_apply(
    ...
    try:
        result = apply_update_maintenance(db, confirm=payload.confirm, release_id=payload.release_id, actor=current_user)
    except UpdateMaintenanceBlocked as exc:
        create_event(
            db=db,
            actor=current_user,
            category="system",
            ...
            severity="warning",
            message_ru="Product update apply was blocked.",
            message_en="Product update apply was blocked.",
            target_type="update_apply",
            metadata={"status": exc.status, "reason": audit_redact_text(str(exc))[:300], "raw_paths_included": False},
            ip_address=request_ip(request),
            user_agent=request_user_agent(request),
        )
    ...
    create_event(
        ...
        actor=current_user,
        category="system",
        event_type="system.update_apply_completed",
        severity="info",
        message_ru="Product update apply completed.",
        message_en="Product update apply completed.",
        target_type="update_apply",
        metadata={
            "status": result.get("status"),
            "release_id": result.get("release_id"),
            "backup_status": result.get("backup_status"),
            "update_apply_executed": result.get("update_apply_executed"),
