...
def _apply_operation_identity(plan: ArchiveIntegrityRemediationPlan) -> tuple[dict[str, str], str]:
    return (
        {"plan_id": plan.id, "canonical_hash": plan.canonical_hash, "action": plan.action_kind},
        hashlib.sha256(plan.id.encode("ascii")).hexdigest(),
    )
...
def apply_remediation_plan(
    ...
    *,
    ...
    if not user_has_permission(str(getattr(actor, "role", "")), plan.required_permission):
        raise IntegrityRemediationBlocked("archive_integrity_permission_denied", retry_mode=None)
    if plan.apply_operation_id is None and plan.expires_at <= database_now(db):
        plan.state = "blocked"
    ...
    root = _root_for_finding(db, finding)
    segment = db.get(RecordingSegment, item.segment_id) if item.segment_id is not None else None
    scope = {
        "global": False,
    ...
    plan.apply_operation_id = str(plan.apply_operation_id or operation_id)
    if plan.state != TERMINAL_PENDING_PLAN_STATE:
        plan.state = "running"
    plan.started_at = plan.started_at or database_now(db)
    plan.updated_at = database_now(db)
    if item.state not in PHYSICAL_PENDING_ITEM_STATES:
        item.state = "running"
...
def recover_pending_remediation_once(db: Session) -> bool:
    """Recover one accepted physical remediation without repeating a committed mutation."""
    plan = (
        db.query(ArchiveIntegrityRemediationPlan)
    ...
    if plan is None:
        plan = (
            ...
            .filter(
                ...
                ArchiveIntegrityRemediationItem.state.in_(tuple(PHYSICAL_PENDING_ITEM_STATES)),
            )
            .order_by(ArchiveIntegrityRemediationPlan.updated_at.asc(), ArchiveIntegrityRemediationPlan.id.asc())
            .first()
        )
    ...
    if plan.state in TERMINAL_PLAN_STATES:
        try:
            return _converge_terminal_plan(db, plan, actor=actor)
    ...
    action = MUTATING_ACTIONS.get(str(plan.action_kind))
    if finding is None or action is None or not plan.apply_operation_id:
        return False
    try:
        root = _root_for_finding(db, finding)
    except IntegrityRemediationBlocked:
...
def recover_terminal_remediation_audit_once(db: Session) -> bool:
    ...
    for plan in plans:
        ...
        if remediation_audit is None:
            actor = db.get(User, int(plan.actor_user_id)) if plan.actor_user_id is not None else None
            _ensure_remediation_terminal_audit(db, plan, actor=actor)
        if operation_audit is None or remediation_audit is None:
            return True
    return False
