...
ENDPOINT_PERMISSIONS: tuple[EndpointPermission, ...] = (
    ...
    EndpointPermission("system", "POST", "/system/restore/current/preflight", PERMISSION_MANAGE_SETTINGS, "require_permission", "Authenticated exact-artifact current DB restore preflight; no current DB replacement."),
    EndpointPermission("system", "POST", "/system/restore/current/apply", PERMISSION_MANAGE_SETTINGS, "require_permission", "Confirmed idempotent current DB restore admission; execution is external."),
    EndpointPermission("system", "GET", "/system/restore/current/status", PERMISSION_MANAGE_SETTINGS, "require_permission", "Actor-bound sanitized current DB restore status."),
    ...
    EndpointPermission("system", "POST", "/system/update/apply", PERMISSION_MANAGE_SETTINGS, "require_permission", "Confirmed idempotent update admission; backend re-resolves the trusted target and remains socket-free."),
    ...
    EndpointPermission("storage", "GET", "/storage/operations/history", PERMISSION_MANAGE_SETTINGS, "require_permission", "Reads the bounded useful archive-operation history shown in Storage management."),
    ...
    EndpointPermission("storage", "POST", "/storage/integrity/scans/{scan_id}/cancel", PERMISSION_RUN_DIAGNOSTICS, "require_permission", "Requests cancellation of the caller's own durable integrity scan."),
    EndpointPermission("storage", "GET", "/storage/integrity/scans/{scan_id}/findings", PERMISSION_RUN_DIAGNOSTICS, "require_permission", "Reads a bounded cursor page of sanitized integrity findings."),
    EndpointPermission("storage", "POST", "/storage/integrity/findings/{finding_id}/metadata-plan", PERMISSION_MANAGE_SETTINGS, "require_permission", "Creates one exact metadata-only remediation plan; no mutation during planning."),
    EndpointPermission("storage", "POST", "/storage/integrity/findings/{finding_id}/deletion-plan", PERMISSION_DELETE_RECORDINGS, "require_permission", "Creates one exact catalog/media deletion plan; no mutation during planning."),
    EndpointPermission("storage", "GET", "/storage/integrity/remediation-plans/{plan_id}", PERMISSION_RUN_DIAGNOSTICS, "require_permission", "Reads the caller's sanitized immutable remediation plan state."),
    EndpointPermission("storage", "POST", "/storage/integrity/remediation-plans/{plan_id}/apply-metadata", PERMISSION_MANAGE_SETTINGS, "require_permission", "Applies one confirmed exact metadata-only plan after full revalidation."),
    EndpointPermission("storage", "POST", "/storage/integrity/remediation-plans/{plan_id}/apply-deletion", PERMISSION_DELETE_RECORDINGS, "require_permission", "Applies one confirmed exact catalog/media plan after full revalidation."),
    EndpointPermission("storage", "GET", "/storage/reconciliation/summary", PERMISSION_RUN_DIAGNOSTICS, "require_permission"),
    EndpointPermission("storage", "POST", "/storage/reconcile", PERMISSION_MANAGE_SETTINGS, "require_permission", "Compatibility scan start only; legacy broad apply is rejected."),
    ...
    EndpointPermission("previews", "GET", "/previews/camera-previews/{camera_id}.jpg", PERMISSION_MANAGE_CAMERAS, 'require_permission("manage_cameras")', "Authenticated preview file response under configured previews root."),
    EndpointPermission("previews", "GET", "/previews/camera-tests/{token}.jpg", PERMISSION_MANAGE_CAMERAS, 'require_permission("manage_cameras")', "Authenticated camera-test preview file response under configured previews root."),
    ...
    EndpointPermission("recordings", "POST", "/recordings/deletion-plans", PERMISSION_DELETE_RECORDINGS, "require_permission", "Creates an actor-bound camera/all deletion plan with an exact server-side immutable manifest."),
    EndpointPermission("recordings", "DELETE", "/recordings/deletion-plans/{plan_id}", PERMISSION_DELETE_RECORDINGS, "require_permission", "Cancels only the caller's ready, unconsumed deletion plan."),
    EndpointPermission("recordings", "POST", "/recordings/deletion-plans/{plan_id}/execute", PERMISSION_DELETE_RECORDINGS, "require_permission", "Executes only the confirmed actor-bound plan scope."),
    EndpointPermission("recordings", "DELETE", "/recordings/by-camera", PERMISSION_DELETE_RECORDINGS, "require_permission"),
    EndpointPermission("recordings", "DELETE", "/recordings/all", PERMISSION_DELETE_RECORDINGS, "require_permission", "Legacy compatibility route; destructive execution requires a server-issued plan."),
