...
`--prepare-version` deliberately clears the previous release `title`, `summary`, `changelog` and localized note maps. Before `--check` can pass for the new version, write new plain `title`/`summary` plus complete `title_i18n` and `summary_i18n` maps for exactly `en`, `ru` and `zh-CN`. `changelog_i18n` is optional, but when present it must contain all three locale lists. Plain text remains the backward-compatible/GitHub source; installed identities copy only note fields that actually exist and never invent a generic English description.
...
Current-database restore keeps its verified pre-restore backup, writer-isolation, database validation, automatic rollback, API health and fresh-recorder gates. Service recovery starts or restarts only the exact existing `api` and `recorder` containers discovered through the current Compose project. It does not use `docker compose start/restart` for recovery, because Compose can re-evaluate an already-removed one-shot dependency such as `schema-update` and leave otherwise recoverable product containers stopped. This container-ID primitive is Docker/Compose based and has no NAS-vendor path or container-name dependency.
